...
function generate(schemaName, types, relayEnabled){
  ...
  all_custom_output_names.forEach(typeName => {
    createUpsertMutationFor(schemaName, typeName, object_types, input_types, interface_types, resolver.resolveUpsert);
    createDeleteMutationFor(schemaName, typeName, object_types, interface_types, resolver.resolveDelete);
  });
}

function createDeleteMutationFor(schemaName, typeName, object_types, interface_types, resolverFunction) {
  ...
  Object.assign(object_types.Mutation._typeConfig.fields, {
    [`${rootMutationFieldName}`]: {
      ...
      type: createDeleteMutationField.createRelayPayloadType(schemaName, typeName, object_types, interface_types),
...
function createUpsertMutationFor(schemaName, typeName, object_types, input_types, interface_types, resolverFunction) {
  let rootMutationFieldName = 'upsert'+typeName;
  Object.assign(object_types.Mutation._typeConfig.fields, {
    [`${rootMutationFieldName}`]: {
      name: rootMutationFieldName,
      type: createUpsertMutationField.createRelayPayloadType(schemaName, typeName, object_types, interface_types),
      description: `Delete an ${typeName} with id and return the ${typeName} that was deleted.`,
      args: {
...
//       //console.log(`Connection of type ${fieldValue.type.name} found in ${outputType.name} for edge ${fieldValue.name}`);
