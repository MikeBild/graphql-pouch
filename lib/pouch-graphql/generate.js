...
let InputObjectType = graphql.GraphQLInputObjectType;
let InterfaceType = graphql.GraphQLInterfaceType;
let ScalarType = graphql.GraphQLScalarType;
let ObjectType = graphql.GraphQLObjectType;
let UnionType = graphql.GraphQLUnionType;
let NonNullType = graphql.GraphQLNonNull;
let EnumType = graphql.GraphQLEnumType;
let ListType = graphql.GraphQLList;
...
function Generate (document, name) {
  let description = [];
  let unions = [];
  ...
  function getDescription() {
    ...
    let prefix = description.reduce(function (a, b) {
      return Math.min(a, /^\s*/.exec(b)[0].length);
    }, Infinity);
    let result = description.map(function (str) {
  ...
  function getInterface(node) {
    ...
    let t = interfaceTypes[node.name.value];
  ...
  function makeResolveType(nodeValue) {
    return function typeResolver(value, context, info) {
      let resolved = resolver(value, context, info);
